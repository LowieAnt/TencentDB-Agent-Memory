...
export function createKnowledgeModule(config: KnowledgeModuleConfig): KnowledgeModule {
  ...
  const realCodeWorker: CodeGraphWorker = async (ctx) => {
    const { dir, repoUrl, branch, credential, codeGraphId, setInternalStatus } = ctx;
    ...
    if (isExistingRepo) {
      try {
        ...
        const res = await fetcher.sync(repoUrl, branch, dir, credential);
        version = res.version;
    ...
    if (!didIncrementalSync) {
      ...
      const res = await fetcher.fetch(repoUrl, branch, dir, credential);
      version = res.version;
